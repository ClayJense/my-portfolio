...
const pageLinks = [
  { href: "/", label: "Accueil", icon: <Home className="size-5" /> },
  { href: "/blog", label: "Blog", icon: <FileText className="size-5" /> },
  { href: "/contact", label: "Contact", icon: <Mail className="size-5" /> },
]
...
export function Navbar() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 flex justify-center pb-4" aria-label="Navigation">
      <Dock
        direction="bottom"
      ...
      >
        ...
        {socialLinks.map(({ href, label, icon }) => (
          <DockIcon key={label}>
            <DockItem href={href} label={label} icon={icon} external />
          </DockIcon>
        ))}
        <div className="mx-1 w-px self-stretch bg-border shrink-0" aria-hidden />
        <DockIcon>
          <div className="group relative flex flex-col items-center size-full">
            ...
            <AnimatedThemeToggler
              ...
              className="text-foreground hover:text-foreground/80 transition-colors [&_svg]:size-5 flex items-center justify-center size-full cursor-pointer"
            />
          </div>
        </DockIcon>
      </Dock>
